...
- **`RefScope.IncludeRefPrefixes` narrows a request to one or more namespaces.** With any prefix set, a ref outside all of them is neither pushed nor pruned, in every kind — the filter is the single predicate the desired set, `TargetScope.Manages` and the prune set all ask, so push scope and prune scope cannot disagree. Exclusions still apply within an included prefix, and explicit `Mappings` bypass both as before. Every entry must start with `refs/`: a blank or `refs/`-less prefix scopes a request to nothing, which under a prune policy is a zero-plan success that repeats forever, so it is rejected at the request edge instead. git-sync's own `refs/gitsync/` scaffolding is the exception to the narrowing: prune is the only cleaner a stale bootstrap marker has, so scaffolding answers to the exclusions alone — except that a bootstrap marker takes its include scope from the branch it checkpoints, so two syncs with different scopes sharing one target cannot delete each other's live resume state. Callers whose target refuses writes outside the included prefixes must exclude `refs/gitsync/` explicitly; scaffolding for branches inside the scope stays prunable. The source ref listing is not narrowed: `ref-prefix` stays `refs/` under `AllRefs`, so an empty in-scope set stays distinguishable from a source that advertised nothing.

- **`SyncPolicy.AllowEmptyScope` lets an empty in-scope desired set prune instead of failing.** Without it, a replicate whose scope matched no source ref fails with the historical "no source refs matched", so a caller replicating a namespace could never propagate the deletion of its last ref. With it, a source that advertised at least one ref proceeds with an empty desired set and the target's in-scope refs are pruned; a source that advertised nothing at all still fails, now with `ErrSourceEmptyUnverified`, because a withheld listing is indistinguishable from an empty one. Replicate-only (only replicate's empty-set branch consults the policy; elsewhere the run returns the historical error and keeps the refs the caller meant to prune), and requires `Prune` (without it the policy plans nothing and reports a green pass over the refs it was set to reap), `AllRefs` (a narrowed listing cannot tell an empty namespace from one it never asked about) and `IncludeRefPrefixes`. Mutually exclusive with `AllowEmptySource`, which needs the unscoped listing its own claim rests on. Every one of these is rejected at the request edge rather than discarded.
...
- **A failed bootstrap now returns its route alongside the error.** `Plan`, `Sync`, and `Replicate`, plus their unstable counterparts and `Bootstrap`, preserve the selected operation and transfer modes, relay fallback reason, attempted ref plan, and completed batch progress when a bootstrap fails, so a failed or partially completed bootstrap can be told apart from a pre-execution failure without reconstructing the route from the error text. This covers the bootstrap route only: a failed incremental or materialized relay still reports an empty transfer mode, and most other error paths still return a zero result. Outcome counts are unpopulated on every one of these paths and must not be used to infer how much landed ([#118](https://github.com/entireio/git-sync/pull/118))
