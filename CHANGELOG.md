...
- **Ref names from a remote are validated before git-sync acts on them.** Nothing checked them, and they reach two places where a malformed name matters: `convert-sha256` writes them to disk, where `refs/heads/../../config` escapes `refs/heads` (go-billy clamps the traversal at the repository root, so this could overwrite `config`, `HEAD`, or `packed-refs` rather than arbitrary files); and ref-update commands embed them in the receive-pack request, where a NUL lets the source inject a capability list into the push git-sync sends to the *target*, since receive-pack reads features from everything after the first NUL on a command line. Advertised names now go through `plumbing.ReferenceName.Validate` — git's own `check_refname_format`, so nothing git accepts is affected — at the two decode boundaries, covering every path that reads an advertisement, on both the source and target side. A name that fails is skipped with a warning naming it rather than failing the run, matching how per-ref push rejections behave under `BestEffort`: one malformed ref upstream should not stop a mirror. Skipped names are printed quoted, so an escape sequence embedded in one is shown rather than interpreted. Ref names supplied through `--map` are validated too, but rejected outright — configuration should fail at startup rather than silently not mirror what was asked for.
...
- **The materialized object limit is enforced while the pack streams, not after.** `FetchToStore` filled the in-memory store with no cap, and `--materialized-max-objects` (default 500,000) was only checked against the object closure once the fetch had finished — by which point the objects were resident and the process may already have died, so the guard reported an overrun it exists to prevent. README's claim that memory is "guarded by an explicit object-count limit" was therefore optimistic. The store is now wrapped so the count is enforced as objects are decoded, failing the write that would exceed the limit; `RawObjectWriter` is the choke point every object routes through. Errors carry an `ErrObjectLimit` sentinel and an `ObjectLimitError` with the limit, and name `--materialized-max-objects` so the message is actionable. The cap applies per fetch rather than cumulatively, because a store outlives one fetch — the materialized tag refill streams into the same store with no `have` lines, so the source resends objects already present, and charging for those twice would fail a run whose distinct object count is within the limit. Worst-case residency is therefore the limit times the number of fetches, two on the sync path. The ceiling applies to the sync path only: `Fetch` exercises source-side negotiation and without `--have` deliberately pulls a full pack, so it is bounded only when a caller sets `MaterializedMaxObjects` — which the unstable `Fetch` path now actually threads through, having previously accepted and dropped it. Reads are untouched, so planning and the push path see the store exactly as before, and the closure check stays as a second line of defence.
...
- **Server-authored text is stripped of terminal control characters.** Sideband progress, up to 64 KiB of HTTP error body, receive-pack `ng` rejection reasons, diagnostic response headers, and ssh's relayed `remote:` output all reached terminals, log files and `--json` results unfiltered. A hostile remote could embed an escape sequence to redraw the line its message was printed on — making a rejected push read as a successful one — or smuggle control characters into whatever ingests the JSON. All five paths now go through `internal/sanitize`, which drops everything below `0x20` plus DEL. Carriage return is kept only for streamed progress, where git's in-place updates depend on it and the `source:`/`target:` line prefix bounds what overwriting a row achieves; one-shot messages lose it, because a bare CR rewrites the line on its own — `rejected\rok refs/heads/main` reads as a success with no escape sequence involved. Filtering is applied to what is actually printed: a rejection error renders its wrapped server status, so cleaning only the `Reason` field would have left the path the CLI displays untouched. Two further carriers are covered — the best-effort rejection callback, whose text becomes `BranchPlan.Reason` and is both printed and marshalled as `reason` (this is the designed surface under `--all-refs`, where per-ref rejections are warnings rather than errors), and the receive-pack unpack status, which is formatted directly rather than wrapped. Rejection classification still runs on the raw status, so filtering cannot change whether a rejection is treated as a concurrent move.
