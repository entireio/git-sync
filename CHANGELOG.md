...
- **`RefScope.IncludeRefPrefixes` narrows a request to one or more namespaces.** With any prefix set, a ref outside all of them is neither pushed nor pruned, in every kind — the filter is the single predicate the desired set, `TargetScope.Manages` and the prune set all ask, so push scope and prune scope cannot disagree. Exclusions still apply within an included prefix, and explicit `Mappings` bypass both as before. The source ref listing is not narrowed: `ref-prefix` stays `refs/` under `AllRefs`, so an empty in-scope set stays distinguishable from a source that advertised nothing.

- **`SyncPolicy.AllowEmptyScope` lets an empty in-scope desired set prune instead of failing.** Without it, a replicate whose scope matched no source ref fails with the historical "no source refs matched", so a caller replicating a namespace could never propagate the deletion of its last ref. With it, a source that advertised at least one ref proceeds with an empty desired set and the target's in-scope refs are pruned; a source that advertised nothing at all still fails, now with `ErrSourceEmptyUnverified`, because a withheld listing is indistinguishable from an empty one. Requires `IncludeRefPrefixes`, and is mutually exclusive with `AllowEmptySource`, which needs the unscoped listing its own claim rests on; both rules are rejected at the request edge rather than discarded.
